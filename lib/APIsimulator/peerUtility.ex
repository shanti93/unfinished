    ...
    quote do
      ...
      def handle_info(:initiate_mine, state) do
        if MapSet.member?(state.roles, :miner) do
          txs = BitcoinCore.findTopUnconfirmedTrans(state.mempool)
          tx_hashes = Enum.reduce(txs, MapSet.new(), fn(x, acc) -> MapSet.put(acc, BitcoinCore.hashTransaction(x)) end)
          coinbase_value = BitcoinCore.coinbaseValueCalculate(state.blockchain, txs)
          ...
          txs = [coinbase_tx | txs]
          block = BitcoinCore.blockTemplate(BitcoinCore.hashOfBestBlock(state.blockchain), txs)
          pid = spawn(BitcoinCore, :doMine, [block, coinbase_addr, state.id])
      ...
      defp init_peer(id) do
        neighbors = BitcoinCore.findInitialNeighbors(id)
        ...
        state = %{
          ...
          mempool: BitcoinCore.newMempool(),
          message_record: BitcoinCore.newMessageRecord(),
          mining_process: nil,
          mining_txs: nil
        }
      ...
      defp handle_transaction_received(transaction, sender, state) do
        tx_hash = BitcoinCore.hashTransaction(transaction)
        unless BitcoinCore.message_seen?(state.message_record, :transaction, tx_hash) do
          ...
          verification_result = BitcoinCore.transactionCheck(new_state.blockchain, transaction)
          if verification_result == :ok do
            ...
            %{new_state | mempool: BitcoinCore.unconfirmedTransAdd(new_state.mempool, transaction, tx_hash)}
          else
      ...
      defp handle_transaction_assembled(transaction, new_wallet, state) do
        verification_result = BitcoinCore.transactionCheck(state.blockchain, transaction)
        if verification_result == :ok do
          tx_hash = BitcoinCore.hashTransaction(transaction)
          ...
          %{state |
            ...
            mempool: BitcoinCore.unconfirmedTransAdd(state.mempool, transaction, tx_hash),
            wallet: new_wallet
          }
        else
          Logger.info("Transaction rejected [reason: #{verification_result}]")
      ...
      defp handle_block_received(block, sender, state) do
        block_hash = BitcoinCore.hashBlockheader(block.header)
        unless BitcoinCore.message_seen?(state.message_record, :block, block_hash) do
          ...
          verification_result = BitcoinCore.blockCheck(new_state.blockchain, block)
          if verification_result == :ok do
            filtered_neightbors = MapSet.delete(new_state.neighbors, sender)
            BitcoinCore.broadcast_message(:block, block, filtered_neightbors, new_state.id)

            {new_blockchain, new_wallet, new_mempool} =
              BitcoinCore.blockAdd(block, new_state.blockchain, new_state.wallet, new_state.mempool, new_state.mining_process, new_state.mining_txs)
      ...
      defp handle_block_mined(block, coinbase_addr, state) do
        verification_result = BitcoinCore.blockCheck(state.blockchain, block)
        new_state =
          if verification_result == :ok do
            ...
            {new_blockchain, new_wallet, new_mempool} = BitcoinCore.blockAdd(block, state.blockchain, new_wallet, state.mempool)
