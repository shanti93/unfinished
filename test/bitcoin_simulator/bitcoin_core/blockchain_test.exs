defmodule BitcoinSimulator.BitcoinCore.BlockchainTest do
  ...
  test "get new blockchain" do
    ...
    genesis_block = %Blockchain.Block{
      header: %Blockchain.BlockHeader{
        ...
        merkleRoot_hash: <<93, 246, 224, 226, 118, 19, 89, 211, 10, 130, 117, 5, 142, 41, 159, 204, 3,
    ...
    blockchain = %{blockchain |
      blocks: Map.put(blockchain.blocks, Blockchain.hashBlockheader(genesis_block.header), genesis_block),
      transactions: Map.new(),
      block_count: 1,
    ...
    target = Blockchain.newBlockchain()
  ...
  test "block header hash" do
    ...
    assert Blockchain.hashBlockheader(header) == <<130, 240, 173, 63, 120, 29, 97, 134, 5, 49, 147, 152, 31, 23, 120, 112, 195,
  ...
  test "transaction hash" do
    ...
    assert Blockchain.hashTransaction(transaction) == <<32, 241, 165, 87, 37, 152, 91, 4, 231, 22, 207, 55, 15, 167, 229, 221, 177,
  ...
  test "merkle root" do
    ...
    assert Blockchain.merkleRoot([tx, tx]) == <<187, 228, 202, 127, 96, 177, 204, 104, 6, 242, 229, 9, 153, 67, 106, 65, 96,
      69, 152, 249, 177, 5, 60, 162, 25, 52, 220, 41, 68, 249, 107, 223>>
  end

  test "merkle root - empty transactions" do
    assert Blockchain.merkleRoot([]) == <<93, 246, 224, 226, 118, 19, 89, 211, 10, 130, 117, 5, 142, 41, 159, 204, 3,
      129, 83, 69, 69, 245, 92, 244, 62, 65, 152, 63, 93, 76, 148, 86>>
  end
  ...
  test "update unspent txout" do
    ...
    unspent_txout = Blockchain.unspentTransUpdate(txs, unspent_txout)
